#!/usr/bin/env python
#
# The Nanycam driver, based on the 
...
class StreamThread(threading.Thread):
    ...
    def formURL(self):
        ...
        self.url = 'http://%s/mjpeg.cgi?' % self.axis.hostname
        if self.axis.username != None :
          self.url += "user=%s&" % self.axis.username
        if self.axis.password != None :
          self.url += "password=%s&" % self.axis.password
        self.url += "channel=0.mjpeg"
    ...
    def authenticate(self):
        ...
        print "password ", self.axis.password
        print "username ", self.axis.username
    ...
    def findBoundary(self):
        '''The string "--video boundary--" is used to denote the start of an image in
        Axis cameras'''
        while(True):
            boundary = self.fp.readline()
            if boundary=='--video boundary--\r\n':
                break
...
class Axis:
    def __init__(self, hostname, username, password, width, height, frame_id, 
        ...
        self.st = None
        self.pub = rospy.Publisher("image_raw/compressed", CompressedImage, self, queue_size=25)
        self.caminfo_pub = rospy.Publisher("camera_info", CameraInfo, self, queue_size=25)
